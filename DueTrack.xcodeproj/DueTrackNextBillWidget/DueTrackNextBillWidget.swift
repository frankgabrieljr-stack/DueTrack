...
struct DueTrackNextBillWidgetEntryView: View {
    ...
    /// Compact inline text shown above the clock.
    private var accessoryInline: some View {
        if let s = entry.snapshot {
            Text("Next: \(s.name) \(formatShortDate(s.dueDate))")
        } else {
            Text("Next bill: none")
        }
    }

    /// Rectangular widget that sits below the clock.
    private var accessoryRectangular: some View {
        VStack(alignment: .leading, spacing: 2) {
            ...
            if let s = entry.snapshot {
                ...
                Text("\(formatAmount(s.amount)) • \(formatShortDate(s.dueDate))")
                    .font(.system(size: 11))
                    .foregroundColor(s.isOverdue ? .red : .secondary)
                    .lineLimit(1)
            } else {
    ...
    /// Circular widget that sits next to the clock.
    ...
    /// Shorter currency string that fits better in circular widgets.
